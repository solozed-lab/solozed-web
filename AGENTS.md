...
Layout.astro 包含完整 meta 标签：Open Graph、Twitter Card、GEO 地理标签（geo.region）、JSON-LD 结构化数据（WebSite + Organization）。
...
- 分支：`feat/docs-audit-fixes`，文档审计修复完成
- 已完成：README.md 与 AGENTS.md 全面审查修复（移除过期内容、补齐缺失文件、修正与实际代码不符的描述）
- 待处理：持续同步文档与实际代码
